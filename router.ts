...
function parseRequestPattern(method: MethodPattern, path: string): RequestPattern {
...
class KRouter<State> {
  ...
  _routeMap: RouteRule<any, State>[] = [];

  _use(method: MethodPattern, path: string, mw: KMiddleware<any, State>) {
    this._routeMap.push({
